...
export function ListNetworksPage() {
  ...
  const columns: ColumnDef<components["schemas"]["Network"]>[] = [
    {
      accessorKey: "name",
      header: "Name",
      cell: ({ row }) => {
        ...
        return (
          <button
            onClick={() => navigate(`/workspaces/${workspaceId}/networks/${id}`)}
    ...
    {
      accessorKey: "netbird.apiUrl",
      header: "API URL",
      cell: ({ row }) => {
        const apiUrl = row.original.netbird?.apiUrl
        return (
  ...
  return (
    <BaseListPage<components["schemas"]["Network"]>
      ...
      emptyStateMessage="No networks configured yet. Create your first network to get started."
      searchColumn="name"
